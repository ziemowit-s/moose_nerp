...
def makeSpine(model, parentComp, compName, index, frac, SpineParams, randomAngles=True):
    ...
    if randomAngles:
        ...
        dendvect = np.array([parentComp.x, parentComp.y, parentComp.z]) - np.array(
            [parentComp.x0, parentComp.y0, parentComp.z0])
    ...
    setSpineCompParams(model, neck, SpineParams.neckdia, SpineParams.necklen, SpineParams.neckRA, SpineParams.spineRM,
                       SpineParams.spineCM)
    ...
    head.x, head.y, head.z = SpineParams.headlen * np.array([head.x0, head.y0, head.z0]) / norm(
        np.array([head.x0, head.y0, head.z0]))
    # head.x0, head.y0 + SpineParams.headlen, head.z0

    setSpineCompParams(model, head, SpineParams.headdia, SpineParams.headlen, SpineParams.headRA, SpineParams.spineRM,
                       SpineParams.spineCM)
    return head, neck
...
def addSpines(model, container, ghkYN, name_soma):
    distance_mapped_spineDensity = {
        (model.SpineParams.spineStart, model.SpineParams.spineEnd): model.SpineParams.spineDensity}
    headarray = []
