    ...
    if tau is not None:
        ...
        if tauScale is not None:
            if tauScale == 'SurfaceArea':
                capool.tau = tau * (
                            np.pi * comp.diameter * comp.length) / 8.478e-11  # normalize to primdend surface area
...
def extract_and_add_difshell(model, shellMode, comp, spine):
    ...
    shape = distance_mapping(params.ShapeConfig, comp)
    shellsparams = CalciumConfig(shellMode=shellMode, increase_mode=shape.ThicknessIncreaseMode,
                                 outershell_thickness=shape.OutershellThickness,
                                 thickness_increase=shape.ThicknessIncreaseFactor, min_thickness=shape.MinThickness)
