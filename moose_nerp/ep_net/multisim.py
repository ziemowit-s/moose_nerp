...
def moose_main(p):
    ...
    param_sim.fname = 'ep' + prefix + stimtype + presyn + '_freq' + str(stimfreq) + '_plas' + str(
        1 if model.stpYN else 0) + fname_part + 't' + str(trialnum)
    print(
        '>>>>>>>>>> moose_main, presyn {} stpYN {} stimfreq {} simtime {} trial {} plotcomps {} tt {} {}'.format(presyn,
                                                                                                                 model.stpYN,
                                                                                                                 stimfreq,
                                                                                                                 param_sim.simtime,
                                                                                                                 trialnum,
    ...
    param_dict = {'syn': presyn, 'freq': stimfreq, 'plas': model.stpYN, 'inj': param_sim.injection_current,
                  'simtime': param_sim.simtime, 'trial': trialnum, 'dt': param_sim.plotdt}
    if stimfreq > 0:
        ...
        for ntype in model.neurons.keys():
            for tt_syn_tuple in model.tuples[ntype].values():
                if model.stpYN:
                    extra_syntab[ntype], extra_plastabset[ntype] = plas_test.short_term_plasticity_test(tt_syn_tuple,
                                                                                                        syn_delay=0,
    ...
    print('$$$$$$$$$$$$$$ paradigm=', model.param_stim.Stimulation.Paradigm.name, ' inj=0? ',
          np.all([inj == 0 for inj in param_sim.injection_current]), 'simtime:', param_sim.simtime)
...
def multi_main(p):
    ...
    if p.ttstr and p.ttGPe:
        sim_params = [(p.freq, p.syn, p.stpYN, trial, p.cond, p.ttGPe, p.ttstr + '_t' + str(trial)) for trial in
                      range(p.trials)]
    elif p.ttstr:
        sim_params = [(p.freq, p.syn, p.stpYN, trial, p.cond, None, p.ttstr + '_t' + str(trial)) for trial in
                      range(p.trials)]
    elif p.ttGPe:
    ...
    print('************* number of processors', max_pools, ' num params', len(sim_params), 'pools', num_pools, 'syn',
          p.syn, 'freq', p.freq, 'ttfile', p.ttstr)
    print(sim_params)
...
def parse_args(commandline, do_exit):
    ...
    parser.add_argument("--syn", '-syn', type=str, default='non',
                        help='optional: synapse type of special input, omit or non for none')
    parser.add_argument("--freq", '-f', type=int, default=0,
    ...
    try:
        args = parser.parse_args(
            commandline)  # maps arguments (commandline) to choices, and checks for validity of choices.
    except SystemExit:
        if do_exit:
