...
### Function used by fit to fit Exponential form of alpha/beta equations to
### steady statey/tau
def alphabetafunction(X):
    # Exp form only:
    arate, avslope, brate, bvslope, = X
    aB = 0
    aC = 0
    bB = 0
    bC = 0
...
bounds = [ratebound, vslopebound] * 2
...
resX = scipy.optimize.differential_evolution(fit, bounds, args=(xInfWolf, xTauWolf), tol=1e-15,
                                             maxiter=10000)  # ,mutation=1.9,seed=12,popsize=60,recombination=0.2)
