from moose_nerp.prototypes.calcium import SingleBufferParams, SinglePumpParams, CellCalcium, ShapeParams
...
CaShellModeDensity = {soma: CAPOOL, dend: CAPOOL,
                      spines: CAPOOL}  # TODO: #FIXME: segfault when dend uses capool and spines are included
...
BufferTotals = {"no_dye": {'Calbindin': 80e-3, 'CaMC': 15e-3, 'CaMN': 15e-3, 'FixedBuffer': 3.5},
...
tree_shape = ShapeParams(OutershellThickness=.1e-6, ThicknessIncreaseFactor=2, ThicknessIncreaseMode=GEOMETRIC,
                         MinThickness=.11e-6)
head_shape = ShapeParams(OutershellThickness=.07e-6, ThicknessIncreaseFactor=2., ThicknessIncreaseMode=LINEAR,
                         MinThickness=.06e-6)
neck_shape = ShapeParams(OutershellThickness=.1667e-6, ThicknessIncreaseFactor=1., ThicknessIncreaseMode=LINEAR,
                         MinThickness=.13e-6)
...
Plas_syn = PlasParams(Name='ampa', highThreshold=highThreshold, lowThreshold=lowThreshold, highDurationThreshold=0.005,
                      lowDurationThreshold=0.025, highFactor='(0.5/' + str(timeStepFactor) + ')',
                      lowFactor='/' + str(lowThreshold - highThreshold) + '/' + str(timeStepFactor))
